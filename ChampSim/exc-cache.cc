...
#ifdef LLC_BYPASS
        if (cache_type == IS_LLC) { // this is a bypass that does not fill the LLC //added
			...
			if((WQ.entry[index].fill_l1d) && (MSHR.entry[mshr_index].fill_l1d != 1))
			  {
                ...
                // is this dirty?
                if (fill_level !=2 && block[set][way].dirty) { //added
		...
		if (RQ.entry[index].type == LOAD) {
		  ...
		  if(fill_level == FILL_L2)
		      ...
		      if(RQ.entry[index].fill_l1d)
			{
             ...
             else if(fill_level == FILL_LLC){ // added 
