#include "cache.h"
#include "set.h"
#include "ooo_cpu.h" // changed here

uint64_t l2pf_access = 0;

void CACHE::handle_back_invalidation(uint8_t cache_type1, uint64_t address1, uint32_t set1, uint32_t way1)
{
  if (block[set1][way1].valid)
  { 

    PACKET pkt;
    pkt.address = address1;


    if (cache_type1 == IS_LLC)
    {
      for (uint32_t i = 0; i < NUM_CPUS; i++)
      {
        // whether to use upper_level_icache or dcache?
        //  we can get the set and way using get_set and get_way(they are member function of class CACHE)
        if (ooo_cpu[i].L2C.check_hit(&pkt)>=0){
          if(ooo_cpu[i].L1D.check_hit(&pkt)>=0){
            ooo_cpu[i].L1D.invalidate_entry(address1);
          }
          else if(ooo_cpu[i].L1I.check_hit(&pkt)>=0){
            ooo_cpu[i].L1I.invalidate_entry(address1);
          }
          ooo_cpu[i].L2C.invalidate_entry(address1);
        }
        else{
          assert(!(ooo_cpu[i].L1D.check_hit(&pkt)>=0)  || !(ooo_cpu[i].L1I.check_hit(&pkt)>=0));
        }
      }
      invalidate_entry(address1);
    }
    else if(cache_type1 == IS_L2C){
      if(ooo_cpu[cpu].L1D.check_hit(&pkt)>=0){
        ooo_cpu[cpu].L1D.invalidate_entry(address1);
      }
      else if(ooo_cpu[cpu].L1I.check_hit(&pkt)>=0){
        ooo_cpu[cpu].L1I.invalidate_entry(address1);
      }
      invalidate_entry(address1);
    }
  }
...
void CACHE::handle_fill()
{
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
  {
    ...
    // ============ call back invalidation here ==========
    // send set,way,cache_type and address(don't know what address) as arguments
    // address = BLOCK[set][way].address
    handle_back_invalidation(cache_type, block[set][way].address, set, way);
    ...
    if (block[set][way].dirty)
    {
      ...
      if (lower_level)
      { // check this part whether it is doing the copying back victim to LLC if IS_L2C
    ...
    /* ===== changing here =====
    else if(cache_type == IS_L2C){  // if (L2) copy back victim to L3 part

      if (lower_level) {  // check this part whether it is doing the copying back victim to LLC if IS_L2C
            if (lower_level->get_occupancy(2, block[set][way].address) == lower_level->get_size(2, block[set][way].address)) {
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
    {
      ...
      { // read hit
        ...
        if (cache_type == IS_LLC)
        {
          ...
          // if (hit && L3) // send block to upper level dcache?
          // way >= 0 means hit
          // upper level is an array whom to send?
